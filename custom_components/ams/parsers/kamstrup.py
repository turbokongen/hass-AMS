...
def parse_data(stored, data):
    ...
    han_data[const.HAN_LIST_VER_ID] = field_type(fields=pkt[33:47], enc=chr)
    for key in const.SENSOR_COMMON_OBIS_MAP:
        if len(const.SENSOR_COMMON_OBIS_MAP[key]) == 2:
            for item in const.SENSOR_COMMON_OBIS_MAP[key]:
                for i in range(len(pkt)):
                    if pkt[i:i + len(item)] == item:
                        ...
                        if pkt[i + len(item)] == 9:
                            ...
                            han_data[
                                const.HAN_METER_DATETIME] = meter_date_time_str
                            _LOGGER.debug(
        ...
        for i in range(len(pkt)):
            if (pkt[i:i + len(const.SENSOR_COMMON_OBIS_MAP[key])] ==
                ...
                # Visible string construct
                if pkt[i + len(const.SENSOR_COMMON_OBIS_MAP[key])] == 10:
                    print(const.SENSOR_COMMON_OBIS_MAP[key], key)
                    v_start = i + len(const.SENSOR_COMMON_OBIS_MAP[key]) + 2
                    v_length = pkt[v_start - 1]
                    v_stop = v_start + v_length
                    han_data["obis_" + key] = (
                    ...
                    _LOGGER.debug(
                        ...
                        key, const.SENSOR_COMMON_OBIS_MAP[key],
                        (i, i + len(const.SENSOR_COMMON_OBIS_MAP[key])),
